...
//! identify an index. With our key-prefix scheme (one SlateDB instance, CFs
//! are prefixes consumed by SlateDB's `PrefixExtractor`) the meaning carries
//! through unchanged.
//!
//! ## Out-of-scope items
...
/// One row of `information_schema.rocksdb_trx`. Populated by `get_all_trx_info()`.
///
...
/// Snapshot of all currently-active transactions. Built from our per-Txn
/// registry; populated as we wrap each `slatedb::DbTransaction` with our
/// engine-side metadata. Returns empty vec rather than error if the engine
/// isn't initialized.
///
...
///
/// SlateDB's txn conflict detection (`IsolationLevel::SerializableSnapshot`)
/// returns `slatedb::ErrorKind::Transaction` at commit on conflict. We catch
/// these and append to the ring buffer for I_S exposure.
///
...
/// Default CF name. With our PrefixExtractor scheme this maps to a specific
/// `cf_id` (default = 0) used for indexes whose CREATE TABLE comment doesn't
/// pick a CF.
/// Original: rdb_global.h:84 — `DEFAULT_CF_NAME`.
pub const DEFAULT_CF_NAME: &str = "default";

/// CF used for the data dictionary (schema metadata, table_id → name,
/// CF-name → cf_id, savepoint markers, 2PC prepare markers, etc.).
/// `cf_id = u32::MAX` reserved so user CFs never collide.
...
/// Deprecated CF name marker — kept for backwards-compatibility parsing only.
...
/// Max prefix length in bytes for indexed columns.
pub const MAX_INDEX_COL_LEN_LARGE: u32 = 3072;
pub const MAX_INDEX_COL_LEN_SMALL: u32 = 767;

// --- error code base ---

/// First MyRocks-specific HA error code. `rdb_global.h:242` — 500.
/// We reuse codes from 550 to avoid collision while ha_rocksdb stays in tree.
...
/// the key prefix: `varint(cf_id) || u32_be(index_id) || memcmp_key`. Our
/// `PrefixExtractor` extracts the `(cf_id, index_id)` portion for SST-level
/// bloom-filter prefix lookups.
///
...
pub struct ExportStats {
    ...
    pub rows_expired: u64,   // populated from native SlateDB TTL expiration count
...
/// IO-stall counters. Maps to SlateDB's metrics (different shape from
/// RocksDB's). Fields whose semantics don't map are reported as 0.
/// Per _DESIGN.md §1 (perf counters → Re-impl).
#[derive(Debug, Clone, Default)]
